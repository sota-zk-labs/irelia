...
impl TaskHandler for VerifyAndRegisterJob {
    ...
    async fn run(self, ctx: WorkerContext) -> impl IntoTaskHandlerResult {
        ...
        let gps_verify_input = extract_gps_input(&self.0.data.sharp_proof.main_proof)?;

        let state = ctx.extensions().get::<State>().unwrap();
        verify_proof_and_register(&state.app_config, &gps_verify_input).await?;

        let mut job_status = self.0.data.job;
        job_status.status = CairoJobStatus::ONCHAIN;
        job_status.validation_done = true;

        state.job_port.update(job_status).await?;
        debug!("=== VERIFY ONCHAIN SUCCESS ===");
        Ok::<(), CoreError>(())
    }
}
