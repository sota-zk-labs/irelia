...
pub fn sample_register_continuous_page_batch(index: u64) -> anyhow::Result<MemoryPageEntries> {
    let file_path = format!("./src/data_samples/data_samples/memory_page_fact_registry/register_continuous_page_batch_{}.json", index);
...
pub fn sample_large_data_register_continuous_page_batch() -> anyhow::Result<MemoryPageEntries> {
    let byte = include_bytes!("../../data_samples/data_samples_friendly_layers/memory_page_fact_registry/large_data_register_continuous_page_batch.json");
